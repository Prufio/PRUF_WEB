...
export default function NewRecord(props) {
                        ...
                        props.prufClient.get
                            ...
                            .call((_error, _result) => {
                                if (_error) {
                                    ...
                                    let root = window.web3.utils.fromWei(
                                        _result.ACTHprice
                                    )
                                    let acth = window.web3.utils.fromWei(
                                        _result.rootPrice
                                    )
                                    setNRCost(
                                        String(Number(root) + Number(acth))
                                    )
                                }
                            })
                    }
    ...
    const IDHolderPrompt = () => {
        ...
        swal({
            title:
                'In order to mint asset tokens, you must first have an ID token.',
            icon: 'warning',
    ...
    const checkAsset = async () => {
        ...
        if (fileSize > 1000000) {
            return swal({
                title:
                    'Document size exceeds 1 MB limit! (' +
                    String(fileSize) +
                    'Bytes)',
                content: link,
                icon: 'warning',
        ...
        if (storageProvider === '1') {
            window.ipfs.add(payload).then((hash) => {
                if (!hash) {
                    console.log(
                        'Something went wrong. Unable to upload to ipfs'
                    )
                    setIpfsActive(false)
                } else {
        ...
        if (storageProvider === '1') {
            ...
            let newAsset = {
                ...
                assetClassName:
                    ...
                    assetClassName
                        .substring(1, assetClassName.length)
                        .toLowerCase(),
                // eslint-disable-next-line react/prop-types
