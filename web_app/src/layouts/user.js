...
export default function Dashboard(props) {
    ...
    const [image, setImage] = React.useState(
        require('assets/img/Sidebar Backgrounds/TracesWB.jpg')
    )
    ...
    const [simpleAssetView] = React.useState(false)
    ...
    window.onload = () => {
        ...
        if (
            hrefStr.includes('0x') &&
            hrefStr.substring(hrefStr.indexOf('0x'), hrefStr.length).length ===
                66
        ) {
            ...
            _prufClient.get
                ...
                .call((_error, _result) => {
                    if (_error) {
                        ...
                        obj.identicon = (
                            <Jdenticon value={obj.id} width="1000" />
                        )
                        obj.identiconLG = (
                            <Jdenticon value={obj.id} width="1000" />
                        )
                                        ...
                                        .call((_error, _result) => {
                                            if (_error) {
                                                ...
                                                obj.assetClassData = {
                                                    name: _result.name,
                                                    root:
                                                        _result.assetClassRoot,
                                                    custodyType:
                                                        _result.custodyType,
                                                    managementType:
                                                        _result.managementType,
                                                    discount: _result.discount,
                                                    referenceAddress:
                                                        _result.referenceAddress,
                                                    extData: _result['IPFS'],
                                                    storageProvider:
                                                        _result.storageProvider,
    ...
    const getMutableData = (
        ...
        if (!assetHeap)
            return console.log('Failed upon reception of:', assetHeap)
        if (!iteration) {
        ...
        if (
            ...
            _prufClient.utils.ipfsFromB32(obj.mutableDataA).then(async (e) => {
                ...
                if (cookies[window.web3.utils.soliditySha3(e)]) {
                    ...
                    for await (const chunk of window.ipfs.cat(e)) {
                        ...
                        assetsWithMutableData.push(obj)
                        setCookieTo(
                            window.web3.utils.soliditySha3(e),
                            JSON.parse(str)
                        )
                        //console.log("EXIT")
                        return getMutableData(
            ...
            mutableDataQuery = window.web3.utils.hexToUtf8(
                obj.mutableDataA +
                    obj.mutableDataB.substring(
                        2,
                        obj.mutableDataB.indexOf('0000000000')
                    )
            )
            console.log(
                `Mutable query at pos ${iteration}: ${mutableDataQuery}`
            )
            ...
            if (cookies[window.web3.utils.soliditySha3(mutableDataQuery)]) {
                        ...
                        try {
                            _arweave.transactions
                                ...
                                .then((e) => {
                                    ...
                                    assetsWithMutableData.push(obj)
                                    setCookieTo(
                                        window.web3.utils.soliditySha3(
                                            mutableDataQuery
                                        ),
                                        tempObj
                                    )
                ...
                xhr.open(
                    'GET',
                    `http://localhost:1984/tx/${mutableDataQuery}`,
                    true
                )
                xhr.send(null)
            }
    ...
    const getEngravings = (
        ...
        if (
            ...
            _prufClient.utils.ipfsFromB32(obj.engravingA).then((e) => {
                engravingQuery = e
                console.log(
                    `Engraving query at pos ${iteration}: ${engravingQuery}`
                )
            ...
            if (cookies[window.web3.utils.soliditySha3(engravingQuery)]) {
                ...
                xhr.open(
                    'GET',
                    `http://localhost:1984/tx/${engravingQuery}`,
                    true
                )
                try {
                    xhr.send(null)
    ...
    const finalizeAssets = (assetHeap, finalizedAssets, iteration) => {
        ...
        obj.name =
            obj.engraving.name || obj.mutableData.name || 'Name Unavailable'
        ...
        if (obj.assetClassData.storageProvider === '2') {
            ...
            for (let i = 0; i < keys.length; i++) {
                const get = () => {
                    if (
                        vals[i].includes('data') &&
                        vals[i].includes('base64')
                    ) {
    ...
    const setupTokenVals = async (willSetup, who, _addr, pruf) => {
        ...
        window.contracts.ID_TKN.methods
            .balanceOf(_addr)
            .call((error, result) => {
                if (error) {
                    console.log(error)
