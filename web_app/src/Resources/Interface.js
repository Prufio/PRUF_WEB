...
async function assembleInterface(contracts) {
  ...
  const interfaceFunctions = {
    calls: [
      ...
      [
        "nodeExtendedData_NoStruct",
        contracts.AC_MGR.methods.getExtAC_data_nostruct,
      ],
    ...
    txns: [
      [
        "verifyRightsHolder",
        contracts.STOR.methods.blockchainVerifyRightsHolder,
      ],
  ...
  if (!contracts)
    return console.error(
      "PRUF_ERR: Failed to build interface. Provided contract network is unreachable."
    );
  else {
