...
function buildWindowUtils() {
  ...
  const _checkAssetExistsBare = async (idxHash) => {
    ...
    await window.contracts.STOR.methods
      ...
      .call(function (_error, _result) {
        ...
        } else if (Object.values(_result)[2] === "0") {
          tempBool = false;
        } else {
          tempBool = true;
        }
  ...
  const _getStatusString = async (_status) => {
    ...
    switch (statusId) {
      ...
      case "70":
        ...
        break;
      default:
        console.error('PRUF_ERR: Status id "', statusId, '"not recognized.');
        break;
    }
      ...
      await window.contracts.A_TKN.methods
        ...
        .call(function (_error, _result) {
          if (_error) {
            ...
            if (
              window.web3.utils.toChecksumAddress(_result) ===
              window.web3.utils.toChecksumAddress(addr)
            ) {
              tempBool = true;
            } else {
              tempBool = false;
  ...
  const _checkNoteExists = async (idxHash) => {
    ...
    await window.contracts.STOR.methods
      ...
      .call(function (_error, _result) {
        if (_error) {
          return console.log("IN ERROR IN ERROR IN ERROR");
        } else if (Object.values(_result)[8] > 0) {
          tempBool = true;
        } else {
          tempBool = false;
        }
  ...
  const _checkCreds = async (acData, AC, addr) => {
    ...
    if (window.contracts !== undefined) {
      ...
      if (custodyType === "Custodial") {
        let addrHash = await window.web3.utils.soliditySha3(
          window.web3.utils.toChecksumAddress(addr)
        );
        await window.contracts.AC_MGR.methods
          ...
          .call((_error, _result) => {
            if (_error) {
              ...
              if (_result === "0" && window.isACAdmin === false) {
                window.authLevel = "Standard User";
                window.isAuthUser = false;
              } else if (_result === "1" && window.isACAdmin === false) {
                window.authLevel = "Authorized User";
                window.isAuthUser = true;
              } else if (_result === "9" && window.isACAdmin === false) {
                window.authLevel = "Robot";
                window.isAuthUser = false;
              } else if (_result === "1" && window.isACAdmin === true) {
                window.authLevel = "Authorized User/AC Admin";
                window.isAuthUser = true;
              } else if (_result === "9" && window.isACAdmin === true) {
                window.authLevel = "Robot/AC Admin";
                window.isAuthUser = false;
              } else if (_result === "0" && window.isACAdmin === true) {
                window.authLevel = "AC Admin";
                window.isAuthUser = false;
              }
              console.log(_result);
        ...
        await window.contracts.ID_TKN.methods
          ...
          .call((_error, _result) => {
            if (_error) {
              ...
              if (Number(_result) === 1 && window.isACAdmin === false) {
                window.authLevel = "Pruf Minter";
                window.isAuthUser = false;
              } else if (Number(_result) !== 1 && window.isACAdmin === true) {
                window.authLevel = "Pruf User/AC Admin";
                window.isAuthUser = false;
              } else if (Number(_result) === 1 && window.isACAdmin === true) {
                window.authLevel = "Pruf Minter/AC Admin";
                window.isAuthUser = false;
              } else if (Number(_result) !== 1 && window.isACAdmin === false) {
                window.authLevel = "Pruf User";
                window.isAuthUser = false;
              }
              console.log(_result);
  ...
  const _determineTokenBalance = async (addr) => {
    if (window.contracts !== undefined && addr !== undefined) {
      ...
      await window.contracts.A_TKN.methods
        ...
        .call((error, result) => {
          if (error) {
            ...
            _assetBal = result; /* console.log("assetBal: ", _assetBal); */
          }
        });
      ...
      await window.contracts.ID_TKN.methods
        ...
        .call((error, result) => {
          if (error) {
            ...
            _IDTokenBal = result; /* console.log("IDTokenBal", _IDTokenBal); */
          }
        });

      await window.contracts.UTIL_TKN.methods
        .balanceOf(addr)
        .call((error, result) => {
          if (error) {
            console.log(error);
          } else {
            _prufTokenBal = window.web3.utils.fromWei(
              result,
              "ether"
            ); /* console.log("prufTokenBal", _prufTokenBal); */
          }
        });
      ...
      if (Number(_IDTokenBal) > 0 && Number(_IDTokenBal) < 2) {
        ...
        Number(_IDTokenBal) === 0 ||
        _IDTokenBal === undefined ||
        _IDTokenBal > 1
      ) {
  ...
  const _getAssetClassTokenInfo = async (addr) => {
    ...
    if (Number(window.balances.assetClassBalance) > 0) {
      ...
      return (window.hasNoAssetClasses = true);
    }
  ...
  const _getAssetTokenInfo = async (addr, pruf) => {
    ...
    if (Number(window.balances.assetBalance) > 0) {
      ...
      for (let i = 0; i < window.balances.assetBalance; i++) {
        await pruf.get.heldAssetAtIndex(addr, i).call((_error, _result) => {
          if (_error) {
            return console.log("IN ERROR IN ERROR IN ERROR");
          } else {
            let resStr;
      ...
      for (let x = 0; x < tknIDArray.length; x++) {
        await pruf.get.assetRecord(tknIDArray[x]).call((_error, _result) => {
          if (_error) {
            console.log("IN ERROR IN ERROR IN ERROR");
          } else {
            if (Number(Object.values(_result)[5]) > 0) {
              ipfsHashArray.push(
                window.utils.getIpfsHashFromBytes32(Object.values(_result)[5])
              );
            } else {
            ...
            if (Number(Object.values(_result)[6]) > 0) {
              noteArray.push(
                "https://ipfs.io/ipfs/" +
                  String(
                    window.utils.getIpfsHashFromBytes32(
                      Object.values(_result)[6]
                    )
                  )
              );
            } else {
        ...
        await pruf.get.assetPriceData(tknIDArray[x]).call((_error, _result) => {
          if (_error) {
            console.log("IN ERROR IN ERROR IN ERROR");
          } else {
            prices.push({
              price: Object.values(_result)[0],
              currency: Object.values(_result)[1],
            });
          }
        });
        //console.log(x)
      ...
      console.log("No assets held by user");
      return obj;
    }
  };
